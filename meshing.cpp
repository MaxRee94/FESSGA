...
void msh::create_FE_mesh(
    ...
    for (auto const& [node_coord, node_idx] : node_coords) {
        int x = node_coord / (densities.dim_y + 1);
        int y = node_coord % (densities.dim_y + 1);

        // Nodes at an extreme of at least one axis are always boundary nodes
        if (x == 0 || y == 0 || x == densities.dim_x || y == densities.dim_y) {
        ...
        for (int x_offset = 0; x_offset < 2; x_offset++) {
            for (int y_offset = 0; y_offset < 2; y_offset++) {
                int cell_is_filled = densities[(x - 1 + x_offset) * densities.dim_y + (y - 1 + y_offset)];
                if (!cell_is_filled) {
                    boundary_node_coords[node_coord] = node_idx;
                    empty = true;
                    no_boundary_nodes++;
    ...
    while (true) {
        ...
        if (force_new_component_search || (i > 1 && x == start_x && y == start_y)) {
            ...
            x = node_coord / (densities.dim_y + 1);
            y = node_coord % (densities.dim_y + 1);
            start_x = x;
            start_y = y;
        ...
        for (auto _offset : offsets) {
            ...
            if (_neighbor_x < 0 || _neighbor_y < 0 || _neighbor_x > densities.dim_x || _neighbor_y > densities.dim_y)
            ...
            if (x == _neighbor_x) // Vertical line
            {
                ...
                if (x < densities.dim_x) right_cell = densities[x * densities.dim_y + y + y_diff];
                if (x > 0) left_cell = densities[(x - 1) * densities.dim_y + y + y_diff];
                if (x == 0) {
                    infeasible = !right_cell; // Infeasible if the line is on the left x-limit and there's no cell to the right
                }
                else if (x == densities.dim_x) {
            ...
            else if (y == _neighbor_y) // Horizontal line
            {
                ...
                if (y < densities.dim_y) up_cell = densities[(x + x_diff) * densities.dim_y + y];
            ...
            _neighbor_coord = _neighbor_x * (densities.dim_y + 1) + _neighbor_y;
    ...
    for (int i = 1; i < ordered_boundary_node_coords.size(); i++) {
        ...
        int node1_x = node1_coord / (densities.dim_y + 1);
        int node1_y = node1_coord % (densities.dim_y + 1);
        int node2_x = node2_coord / (densities.dim_y + 1);
        int node2_y = node2_coord % (densities.dim_y + 1);
        int cell_x, cell_y;
        ...
        if (node1_x == node2_x) {   // Vertical line
            ...
            if (node1_x == densities.dim_x || !densities[cell_x * densities.dim_y + cell_y]) {
                cell_x = node1_x - 1;
                local_line_idx = 2;
            }
        }
            ...
            if (node1_y == densities.dim_y || !densities[cell_x * densities.dim_y + cell_y]) { 
                cell_y = node1_y - 1;
                local_line_idx = 1;
            }
        }
        // Use the last two bits of the integer to store the line idx. The first 30 are used for the cell coordinates
        cell_coord = cell_x * densities.dim_y + cell_y;
